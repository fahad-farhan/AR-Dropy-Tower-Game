...
namespace _DroppyTower
{
    public class PlayerController : MonoBehaviour
    {
        ...
        void Start()
        {
            bottomPosition = Camera.transform.position;
        ...
        void Update()
        {
            ...
            if(die)
            {
                if (moveDown)
                {
                    Camera.transform.position = Vector3.SmoothDamp(Camera.transform.position, bottomPosition, ref velocity, GameManager.Instance.CameraSpeed);
                    if (Mathf.Abs(Camera.transform.position.y - bottomPosition.y)<1f)
                        moveDown = !moveDown;
                }
                else
                {
                    Camera.transform.position = Vector3.SmoothDamp(Camera.transform.position, topPosition, ref velocity, GameManager.Instance.CameraSpeed);
                    if(Mathf.Abs(Camera.transform.position.y - topPosition.y) < 1f)
                        moveDown = !moveDown;
                }
            }
        }
        ...
        {
            GameObject addCoinCanvas = (GameObject)Instantiate(addCoin, position, Quaternion.Euler(Camera.transform.eulerAngles.x, 0, 0));
        ...
        public void Die()
        {
            ...
            if (PlayerDied != null)
                PlayerDied();
            topPosition = Camera.transform.position;
            // Fire event
        }
